...
export default function Sidebar() {
  ...
  const isSubmenuItemActive = (submenuItems) => {
    return submenuItems.some(item => window.location.pathname === item.path)
  }
    ...
    <div className={`w-64 h-full flex flex-col ${isDark ? 'bg-gray-900' : 'bg-white'} border-r ${isDark ? 'border-gray-700' : 'border-gray-200'} shadow-lg`}>
      ...
      <div className={`p-6 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 bg-[#FFFFFF] rounded-xl flex items-center justify-center shadow-md hover:shadow-lg transition-shadow duration-200 p-2">
            <img 
              src={logoImage} 
              alt="Pack Tamam Logo" 
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-200 group ${
                      isSubmenuItemActive(item.submenu)
                        ? 'bg-[#c79e73] text-white shadow-md'
                        : isDark
